"""PowerFlow — Monthly Brief Generator.

Streamlit UI for the briefing agent. Fetches the last 30 days of Notion data,
...
st.title("📆 PowerFlow — Monthly Brief")
st.caption("Synthesize a structured intelligence brief from events dated in the last 30 days.")

# ── Session state init ────────────────────────────────────────────────────────

if "monthly_data" not in st.session_state:
    st.session_state.monthly_data = None
if "monthly_brief_text" not in st.session_state:
    st.session_state.monthly_brief_text = None
if "monthly_saved_url" not in st.session_state:
...
data: dict = st.session_state.monthly_data
...
if generate_btn:
    ...
    with st.spinner("Synthesizing brief…"):
        try:
            st.session_state.monthly_brief_text = writer.generate_brief(data, priority)
        except Exception as exc:
            st.error(f"Brief generation failed: {exc}")

# ── Brief preview & approval ──────────────────────────────────────────────────

if st.session_state.monthly_brief_text:
    st.markdown("---")
    st.subheader("📄 Generated Brief")

    st.markdown(st.session_state.monthly_brief_text)
    ...
    with col_approve:
        approve_btn = st.button(
            ...
            disabled=bool(st.session_state.monthly_saved_url),
        )
        ...
        with st.spinner("Saving to Notion…"):
            try:
                page_id, page_url = writer.save_brief(
                    brief_text=st.session_state.monthly_brief_text,
                    date_range=date_range,
                    priority=priority,
                    date_range_start=date_range_start,
                )
                st.session_state.monthly_saved_url = page_url
    ...
    if st.session_state.monthly_saved_url:
        st.success(
            f"✅ Brief saved to Notion! [Open page →]({st.session_state.monthly_saved_url})"
        )
